...
// NOTE: you never used min2 or min4, if required, use std:min the same way I used std::max
...
int main()
{
	FILE *fp = fopen("output.xml", "w"); // <-- NOTE: changed output.txt to .xml
	if (fp == NULL)
	{
...
double initial_flow(double x_int, double y_int, double z0_int)
{
	...
	if (x_int <= 16) {
		...
		h = std::max(0.0, etta - z0_int); 
	}
	else{
		//h = 0.0*max2(0.0, etta - z0_int); <-- NOTE: this is always zero?
		h = 0.0;
	}
...
double bed_data(double x_int, double y_int)
{
	...
	//zz = 1.0 * max4((double)zb1, (double)zb2, (double)zb3, (double)zb4); <-- NOTE: you do not need max4. You can use std::max from algorithm library
