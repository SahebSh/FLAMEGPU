...
__FLAME_GPU_STEP_FUNC__ void DELTA_T_func()
{
	...
	// This function is executed within each iteration

	//get the old values
	double old_sim_time = *get_sim_time();
	double old_dt = *get_dt(); //activated to load dt from 0.xml
	...
	//double minTimeStep = 0.005; // for static time-stepping and measuring computation time //commented to load from 0.xml
	...
	printf("Elapsed simulation time = %f \n", new_sim_time); //prints simulation time 
...
__FLAME_GPU_FUNC__ int PrepareSpaceOperator(xmachine_memory_FloodCell* agent, xmachine_message_SpaceOperatorMessage_list* SpaceOperatorMessage_messages)
{
	...
	if ( (INFLOW_BOUNDARY == NORTH) && (sim_time >= FLOOD_START_TIME))
	{
	...
	//If discharge is input on an original dry cell provide a 0.01m water depth to support the discharge !!
	//if ((fabs(agent->h) < TOL_H) && (agent->qx > emsmall))
	//	if ( fabs(agent->h) < TOL_H ) // another condition should be added in case of inflow boundary is activated!
	//{
	//	agent->h = 0.01;
	//	agent->minh_loc = 0.01;
	//}
