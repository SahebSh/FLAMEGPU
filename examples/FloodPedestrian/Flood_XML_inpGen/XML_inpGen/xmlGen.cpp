...
int main()
{
	...
	int auto_dt_on		= ON;	// adaptive time-stepping for flood model
	int body_effect_on	= ON;	// taking pedestrians as moving obstacles
	...
	double DISCHARGE_INITIAL	= 0.0;	// (unit: m2/s)
	double DISCHARGE_PEAK		= 1.3;
	double DISCHARGE_END		= 0.0;
	...
	float sandbag_length = 0.50;
	float sandbag_height = 0.10;
	float sandbag_width  = 0.25;

	// the details of proposed sandbag dike (in metre)
	float dike_length	 = 120;
	float dike_height	 = 0.5;
	float dike_width	 = 3.0; // this is not considered to be taken into account YET (for the current model)
...
double bed_data(double x_int, double y_int)
{
	...
	double left_road_width  = 15; // imaginary width of left-side roads (m)
	double right_road_width = 20;

	// defining corridor specification
	// left side
	double x0 = 0;
	double x1 = 0.2*xmax; //50;
	double y0 = 0.1*ymax; //25;
	double y1 = 0.2*ymax; //50;
	//right side
	double x00 = 250;
	double x11 = 175;
	double y00 = 25;
	...
	double y1_1 = 180;
	double y1_2 = y1_1 + right_road_width;
	//Exit 2
	double y2_1 = 70;
	double y2_2 = y2_1 + right_road_width;//90;

	// exit 3-5 are located on the left side of the road
	//Exit 3
	double y3_1 = 60;
	double y3_2 = y3_1 + left_road_width;//75;
	//Exit 4
	double y4_1 = 130;
	double y4_2 = y4_1 + left_road_width;//145;
	//Exit 5
	double y5_1 = 190;
	double y5_2 = y5_1 + left_road_width;// 205;
