...
double bed_data(double x_int, double y_int)
{
	...
	//***************************************************************************************************//
	//// Coridor topography
	// This function generates a corridor surrounded by walls and buildings containing imaginary roads on each side

	// Constant values evaluated based on xmin = xmax= 250 m

	double left_bound_width  = 15; // imaginary width of left-side roads (m)
	double right_bound_width = 20;

	// defining corridor specification
	// left side
	double x0 = 0;
	double x1 = 0.2*xmax; //50 for default:250*250 size;
	double y0 = 0.1*ymax; //25 for default:250*250 size;
	double y1 = 0.2*ymax; //50 for default:250*250 size;
	//right side
	double x00 = xmax; // 250 for default:250*250 size;
	double x11 = xmax - (0.3*xmax); // 175 for default:250*250 size;
	double y00 = 0.1*ymax; //25 for default:250*250 size;
	double y11 = 0.2*ymax; //50 for default:250*250 size;

	// Exit 1 and 2 are located on the right side of the domain
	//Exit 1 location
	// to specify where the roads are located within among the buildings
	// ******* NOTE : some default constants are estimated based on their initial value of initial default condition

	double y1_1 = ((180.0f*ymax)/250) +5.0f; //180 for default:250*250 size; 180 is taken constant , 5.0f is added for synchornising with pedestrian nav map, 
	//double y1_1 = 239.04; //180 for default:250*250 size; 180 is taken constant
	double y1_2 = y1_1 + right_bound_width;
	//Exit 2
	double y2_1 = (70.0f*ymax)/250; //70 for default:250*250 size; 70 is taken constant
	double y2_2 = y2_1 + right_bound_width;//90;

	// exit 3-5 are located on the left side of the road
	//Exit 3
	double y3_1 = (60.0f*ymax)/250; //60 for default:250*250 size; 60 is taken constant;
	double y3_2 = y3_1 + left_bound_width;//75;
	//Exit 4
	double y4_1 = (130.0f*ymax)/250;// 130 for default:250*250 size; 130 is taken constant;
	double y4_2 = y4_1 + left_bound_width;//145;
	//Exit 5
	double y5_1 = (190.0f*ymax)/250; ; // 190 for default:250*250 size; 190 is taken constant;
	double y5_2 = y5_1 + left_bound_width;// 205;


	// defining left-side rows of buildings containing Exit 3,4, and 5
	if ((x_int <= x1) && (x_int <= (((y_int - y0)*(x1 - x0)) / (y1 - y0)) + x0) )
	{
		//area 1
		if (y_int <= y3_1)
		{
		...
		else if (y_int >= y3_2 && y_int <= y4_1)
		{
			zz = 10;//10
			
		}
		else if (y_int >= y4_2 && y_int <= y5_1 )
		{
			zz = 10;//10
		}
		else if (y_int >= y5_2 )
	...
	// Defining right-side buildings and streets 
	else if ((x_int >= x11) && (x_int >= (((y_int - y00)*(x11 - x00)) / (y11 - y00)) + x00))
	{	
		if (y_int <= y2_1)
		{
			zz = 10;//10
		}
		else if (y_int >= y2_2 && y_int <= y1_1)
		{
			zz = 10;//10
		}
		else if (y_int >= y1_2)
